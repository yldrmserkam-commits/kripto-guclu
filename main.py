...
# 🚀 Mum Çekme Fonksiyonu (Ichimoku 52 periyot geriye baktığı için limit 150 yapılmıştır)
def binance_klines_cek(symbol, interval, limit=150):
  ...
  for url in urls:
    try:
      ...
      if response.status_code == 200:
        data = response.json()
        min_gerekli = 60  # Ichimoku Senkou Span B için en az 52+26 mum gerekebilir
...
for periyot_adi, aktif_mi in TARAMA_YAPILACAK_PERIYOTLAR.items():
  ...
  for ticker in tqdm(tickers, desc=f'{periyot_adi} Taranıyor'):
    df = binance_klines_cek(
        ticker,
        PERIYOT_AYARLARI[periyot_adi]['interval'],
        PERIYOT_AYARLARI[periyot_adi]['limit'],
    )
    time.sleep(0.04)
    ...
    try:
      ...
      # Senkou Span A & B (26 periyot ileri kaydırılır)
      ...
      # RSI Koşulu (70'i yeni geçmiş: Tam kesişim veya 1 mum önce geçmiş)
      ...
      # Ichimoku Bulut Koşulu (Kapanış fiyatı bulutun üstünde olmalı)
      if ICHIMOKU_FILTRESI_AKTIF and close_curr <= bulut_ust:
        continue

      # Hacim Filtresi Koşulu
      ...
      # 🔗 TradingView Vadeli Link Formatı
...
if results:
  ...
  df_results = df_results.sort_values(
      by=['Zaman Dilimi', 'Coin']
  ).reset_index(drop=True)
  df_results.to_excel(
      'Binance_Ichimoku_RSI_Sonuclari.xlsx', index=False
  )
  print(
      f'\n✅ Toplam {len(results)} coin filtrelere ulaştı ve Excel'
      'ye kaydedildi.'
  )
else:
